...
状态：已完成
日期：2026-08-22（2026-08-23 完成时长自适应采样）
...
- 在服务端使用已有 ffmpeg/ffprobe 基础设施，根据视频时长按约 2.5 秒目标间隔
  规划时间线采样；最少三张、最多二十四张，最长边不超过 1280 像素。
...
- 本次不引入镜头切换检测、长视频分段、帧缓存或新的 Tool 参数。
...
1. `video_analyze` 发给 LLM 的内容包含按时长规划的有界 `image_url`，不包含
   `video_url`。
2. 每张时间线帧携带可读时间码；短视频至少三张，较长视频增加覆盖但不超过
   二十四张。
